...
export default function AboutPage() {
  ...
  const paragraphs: string[] = (() => { try { return JSON.parse(data.about_paragraphs); } catch { return []; } })();
  const advantages: string[] = (() => { try { return JSON.parse(data.about_advantages); } catch { return []; } })();

  return (
    <PublicPage emphasizeBackground>
      <section className="mx-auto w-full max-w-[1160px] px-4 py-8 sm:px-6 lg:px-8">
        <PageSection title={data.about_hero_title} description={data.about_hero_description} />
