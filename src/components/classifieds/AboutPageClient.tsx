...
export function AboutPageClient() {
  ...
  const title = t("about_hero_title", "Platforma haqqında");
  const description = t("about_hero_description", "Tranzit.AZ yük bazarında əlaqəni sürətləndirən, aydın və rahat iş axını təqdim edən elan platformasıdır.");
  const paragraphs = tArray("about_paragraphs", DEFAULT_PARAGRAPHS);
  const advantages = tArray("about_advantages", DEFAULT_ADVANTAGES);
  const advantagesTitle = t("about_advantages_title", "Əsas üstünlüklər");
    ...
    <PublicPage emphasizeBackground>
      <section className="mx-auto w-full max-w-[1160px] px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 space-y-6">
          ...
          <StaticContentCard>
            <h2 className="text-[1.45rem] font-bold text-navy-900">{advantagesTitle}</h2>
            <div className="mt-5">
